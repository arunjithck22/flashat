...
import { useInviteToSpeak } from "@/app/hooks/podiums/useInviteToSpeak";
...
const InviteToSpeakConfirmationModal = ({
  closeModal,
  participantId,
}: {
  closeModal: () => void;
  ...
  const inviteToSpeakMutation = useInviteToSpeak();

  const handleInviteToSpeak = () => {
    inviteToSpeakMutation.mutate(
      {
        data: {},
  ...
  return (
    <div className="flex flex-col w-full min-w-[250px] max-w-[200px] sm:min-w-72 sm:max-w-96 ">
      <div className="flex flex-col gap-2">
        <h3 className="text-md">{t("invite_to_speak_confirmation")}</h3>
      </div>
      <div className="flex w-full  justify-end   gap-3 ">
        ...
        <Button
          ...
          disabled={inviteToSpeakMutation.isPending}
          onClick={() => {
            handleInviteToSpeak();
          }}
...
export default InviteToSpeakConfirmationModal;
