...
const DismissConfirmationModal = ({
  ...
  const dismissAdminMutation = useDismissFromAdminPodium();

  const handleDismissAdmin = () => {
    dismissAdminMutation.mutate(
      {
        data: {},
  ...
  return (
    <div className="flex flex-col w-full min-w-[250px] max-w-[200px] sm:min-w-72 sm:max-w-96 ">
      <div className="flex flex-col gap-2">
        <h3 className="text-md">{t("dismiss_admin_confirmation")}</h3>
      </div>
      <div className="flex w-full  justify-end   gap-3 ">
        ...
        <Button
          ...
          disabled={dismissAdminMutation.isPending}
          onClick={() => {
            handleDismissAdmin();
          }}
